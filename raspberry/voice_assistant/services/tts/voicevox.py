...
class TTSPlayer:
    ...
    def __init__(self):
        ...
        self._full_text = ""
        self._mouth_timeline: list[float] = []
        ...
        self.on_start_cb: Callable[[], None] | None = None
    ...
    def get_mouth_openness(self) -> float:
        """Return mouth openness 0.0-1.0 based on current audio RMS timeline, -1.0 if not playing."""
    ...
    def cancel(self) -> None:
        ...
        if proc and proc.poll() is None:
            try:
                proc.terminate()
                proc.wait(timeout=0.5)  # 音声出力が完全に止まるまで待つ
    ...
    def _fetch_loop(self) -> None:
        while True:
            ...
            if not original_text:
                continue
            filtered_text = apply_tts_filter(original_text)
            if not filtered_text:
                continue
            wav_data = self._fetch_wav(filtered_text)
            if self._cancel.is_set():
                self._play_q.put(_SENTINEL)
                continue
            if wav_data:
                # 表示は変換前テキスト、音声は変換後テキストを使用
                self._play_q.put((original_text, wav_data))
            else:
                print(f"[tts_voicevox] skipping sentence (fetch failed): {original_text[:40]}")
    ...
    def _fetch_wav(self, text: str) -> bytes | None:
        ...
        # config.OPENAI_TTS_SPEED -> voicevox speedScale
        query_data["speedScale"] = max(0.5, min(2.0, config.OPENAI_TTS_SPEED))
        # ハードウェアのネイティブレートで出力させてリサンプリングを排除
        query_data["outputSamplingRate"] = config.VOICEVOX_SAMPLE_RATE
    ...
    def _play_wav(self, wav_data: bytes) -> None:
        ...
        try:
            ...
            # stdin.write を専用スレッドに分離する。
            # シングルコアの Pi Zero では、大量のパイプ I/O をメインの再生スレッドから
            # 切り離すことで、アニメーションスレッドへの CPU 圧迫を軽減できる。
            def _feed():
                try:
                    proc.stdin.write(wav_data)
                    proc.stdin.close()
                except (BrokenPipeError, OSError):
                    pass  # cancel() でプロセスが kill された場合は無視
            ...
            print("[tts_voicevox] aplay not found — install alsa-utils")
        except Exception as e:
            print(f"[tts_voicevox] playback error: {e}")
        finally:
            # is_speaking はここではクリアしない。
            # センテンス間のギャップでアバターが止まるのを防ぐため、
            # _play_loop がセンチネルを受け取ったときにのみクリアする。
            self._mouth_timeline = []


def _analyze_mouth(wav_data: bytes) -> list[float]:
    """Parse WAV and compute mouth shape (0.0-1.0) per 80ms window."""
    header_size = 44
    ...
    def _rms_to_openness(rms: float) -> float:
        if rms < 300:
            return 0.0
        return min(1.0, (rms - 300) / 3700.0)
    ...
    if _HAS_NUMPY:
        ...
        for offset in range(0, len(arr) - samples_per_window + 1, samples_per_window):
            ...
            shapes.append(_rms_to_openness(rms))
    else:
